  ...
  try {
    ...
    const [lotes, contratos, clientes, pagos, equipo, documentos, comisiones, adjuntos, recibos, liquidaciones, requeridos, proyectos, personaProyectos] = await Promise.all([
      ...
      conRespaldo('persona', 'id,nombre,codigo,rol,email,telefono,activo',
                  'auth_uid,externo,organizacion,acceso_hasta,vendedor_hasta,exige_2fa'),
      ...
      conRespaldo('proyecto', 'id,codigo,nombre,municipio,departamento,tasa_mensual,tasa_mora,enganche_minimo,plazos,comision_pct,dias_gracia', 'moneda,tipo_cambio,metodo_interes,tasa_anual,enganche_pct,expediente_robusto'),
    ...
    (proyectos || []).forEach(pr => {
      ...
      PROYECTOS[pr.codigo] = Object.assign({ corto: pr.nombre, activo: true }, base, {
        ...
        tasaMensual: _num(pr.tasa_mensual) || base.tasaMensual, tasaMora: pr.tasa_mora != null ? _num(pr.tasa_mora) : base.tasaMora, diasGracia: pr.dias_gracia != null ? _num(pr.dias_gracia) : (base.diasGracia || 0),
...
async function cargarCartera() {
  ...
  try {
    const [obligaciones, giros] = await Promise.all([
      ...
      conRespaldo('giro', 'id,obligacion_id,numero,vencimiento,monto,estado,abonado', 'condicion,fecha_estimada,capital,interes,interes_exonerado')
    ]);
